<script lang='ts'>
  ...
  import { DataTable, Button, Toolbar, ToolbarContent, ToolbarSearch, ToolbarBatchActions } from "carbon-components-svelte"
...
// Rows
  let rows = []
  let searchString = ''
  $: displayedRows = rows.filter(row => `${row['name']}${row['creator']}`.search(searchString) > -1)
